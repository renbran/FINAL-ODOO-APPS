...
export class CRMStrategicDashboard extends Component {
  ...
  async loadInitialData() {
    try {
      await Promise.all([this.loadDashboardData(), this.loadTeamsList()]);
    } catch (error) {
  ...
  initializeCharts() {
    ...
    const initCharts = () => {
      if (typeof Chart !== "undefined") {
        ...
        console.warn(
          "Chart.js not available for strategic dashboard, using fallback"
        );
  ...
  async onRefreshClick() {
    ...
    this.notification.add(_t("Dashboard data refreshed successfully"), {
      type: "success",
    });
  }
  ...
  async exportData() {
    try {
      const url = `/crm/strategic/dashboard/export?date_from=${
        this.state.dateRange.start
      }&date_to=${
        this.state.dateRange.end
      }&team_ids=${this.state.selectedTeams.join(",")}`;
      window.open(url, "_blank");
    } catch (error) {
      this.notification.add(_t("Failed to export data: %s", error.message), {
        type: "danger",
      });
    }
  }
...
registry
  .category("actions")
  .add("crm_strategic_dashboard", CRMStrategicDashboard);
