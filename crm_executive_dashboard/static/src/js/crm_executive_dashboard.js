...
export class CRMExecutiveDashboard extends Component {
  ...
  renderSourcesChart() {
    ...
    if (
      !canvas ||
      !this.state.dashboardData.customer_acquisition.sources.labels
    )
      return;
    ...
    this.action.doAction({
      ...
      domain: [
        [
          "groups_id",
          "in",
          [this.env.ref("sales_team.group_sale_salesman").id],
        ],
      ],
    });
  }
  ...
  async exportData() {
    try {
      const url = `/crm/dashboard/export?date_from=${
        this.state.dateRange.start
      }&date_to=${
        this.state.dateRange.end
      }&team_ids=${this.state.selectedTeams.join(",")}&format=xlsx`;
...
registry
  .category("actions")
  .add("crm_executive_dashboard", CRMExecutiveDashboard);
