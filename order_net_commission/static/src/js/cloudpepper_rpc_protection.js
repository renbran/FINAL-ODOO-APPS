...
window.addEventListener("error", function (event) {
  if (event.error && event.error.message) {
    console.warn(
      "Order Net Commission - Caught global error:",
      event.error.message
    );

    // Handle specific RPC errors
    if (
      event.error.message.includes("RPC_ERROR") ||
...
window.addEventListener("unhandledrejection", function (event) {
  if (event.reason && event.reason.message) {
    console.warn(
      "Order Net Commission - Caught unhandled promise rejection:",
...
registry
  .category("services")
  .add("order_net_commission_protection", rpcService);
  ...
  mounted() {
    ...
    try {
      if (
        this.props &&
        this.props.record &&
        this.props.record.model === "sale.order"
      ) {
...
(function () {
  ...
  const CloudPepperProtection = {
    ...
    init: function () {
      console.log(
        "Order Net Commission protection initialized for CloudPepper"
      );
    },
  };
...
console.log(
  "Order Net Commission - CloudPepper RPC protection loaded successfully"
);
