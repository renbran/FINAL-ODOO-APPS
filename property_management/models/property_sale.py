...
class PropertySale(models.Model):
    ...
    property_sale_offer_id = fields.Many2one('property.sale.offer', string='Property Offer', ondelete='set null')
    ...
    def action_generate_broker_commission_invoice(self):
        ...
        if self.property_sale_offer_id:
            commission = self.env['broker.commission.invoice'].create({
                ...
                'property_sale_offer_id': self.property_sale_offer_id.id,
    ...
    def action_cancel(self):
        for record in self:
            ...
            if record.property_sale_offer_id and record.property_sale_offer_id.state == 'accepted':
                record.property_sale_offer_id.write({'state': 'sent'})
    ...
    def create(self, vals):
        ...
        if vals.get('property_sale_offer_id'):
            offer = self.env['property.sale.offer'].browse(vals['property_sale_offer_id'])
...
class PropertySaleLine(models.Model):
    ...
    property_sale_offer_id = fields.Many2one(related='property_sale_id.property_sale_offer_id', string='Property Offer', store=True, readonly=True)
