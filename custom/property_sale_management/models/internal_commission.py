...
class InternalCommission(models.Model):
    ...
    position = fields.Selection([
        ...
        ('director', 'Director'),
        ('other', 'Other')
    ], required=True, string="Role Position", tracking=True)
    ...
    percentage = fields.Float(string="Percentage (%)", digits=(5, 2), tracking=True)
    ...
    def write(self, vals):
        if 'percentage' in vals or 'partner_id' in vals:
            if any(rec.is_billed for rec in self):
                raise UserError(_("Cannot modify percentage or recipient for a billed commission."))
        return super(InternalCommission, self).write(vals)

    def unlink(self):
        if any(rec.is_billed for rec in self):
            raise UserError(_("Cannot delete a billed commission allocation."))
        return super(InternalCommission, self).unlink()

    @api.model
    def create(self, vals):
        res = super(InternalCommission, self).create(vals)
        res.broker_commission_id.message_post(
            body=_("New commission allocation created for %s with percentage %s%%") % (res.partner_id.name, res.percentage)
        )
        return res