...
class AccountPayment(models.Model):
    ...
    def _compute_amount_in_words(self):
        ...
        for record in self:
            if record.amount and record.currency_id and num2words:
                try:
                    # Get currency name
                ...
                record.amount_in_words = f"{record.currency_id.symbol if record.currency_id else ''} {record.amount:,.2f}" if record.amount else ''
    ...
    def _compute_workflow_progress(self):
        ...
        for record in self:
            if record.voucher_type == 'payment':
                # Payment Voucher: 6 stages (Draft, Submitted, Review, Approved, Authorized, Posted)
                progress_map = {
                    'draft': 0,
                    'submitted': 16.67,
                    'under_review': 33.33,
    ...
    def _compute_is_locked(self):
        """Compute if record is locked for editing"""
        for rec in self:
    ...
    def create(self, vals):
        ...
        try:
            ...
            _logger.warning(f"Could not create QR verification record: {e}")
    ...
    def action_post(self):
        """Overwrites the action_post() to validate the payment in the 'approved' stage too."""
        ...
        # Call the parent's action_post method
        return super(AccountPayment, self).action_post()

    def _check_payment_approval(self):
        """This function checks the payment approval if payment_amount greater than amount"""
        self.ensure_one()
        if self.state == "draft":
            ...
            if first_approval:
                ...
                payment_currency_id = self.env['ir.config_parameter'].sudo().get_param(
    ...
    def approve_transfer(self):
        """This function changes state to approved state if approving person approves payment"""
        for record in self:
    ...
    def action_reject(self):
        ...
        rejection_reason = ctx.get('rejection_reason', 'No reason provided')
