  ...
  window.PaymentWorkflowRealtime = {
    ...
    setupWorkflowObservers: function () {
      try {
        ...
        $(document).on(
          "change",
          'select[name="reviewer_id"], select[name="approver_id"], select[name="authorizer_id"]',
          function () {
            try {
    ...
    updateWorkflowProgress: function (currentState) {
      ...
      $progress.find(".progress-step").each(function (index) {
        ...
        if (index <= currentIndex) {
          $step
            .removeClass("step-pending step-current")
            .addClass("step-completed");
        } else if (index === currentIndex + 1) {
          $step
            .removeClass("step-pending step-completed")
            .addClass("step-current");
        } else {
          $step
            .removeClass("step-completed step-current")
    ...
    updateFieldsFromServer: function (data) {
      ...
      console.log(
        "Direct field updates from server disabled for CloudPepper compatibility"
      );
    },
    ...
    showNotification: function (message, type, duration) {
      try {
        ...
        var $notification = $(
          '<div class="alert alert-' +
            type +
            ' payment-notification" style="position: fixed; top: 20px; right: 20px; z-index: 9999; max-width: 400px;">' +
      ...
      if ($(e.target).find('select[name="approval_state"]').length > 0) {
        setTimeout(function () {
          try {
            ...
            console.log(
              "PaymentWorkflowRealtime re-initialization error:",
              error
            );
          }
        }, 100);
