...
(function() {
    ...
    if (typeof window.odoo.define !== 'function') {
        window.odoo.define = function(name, dependencies, callback) {
            console.warn(`[Modern Odoo17] Legacy odoo.define() call detected for "${name}". Please modernize to ES6+ modules.`);
            ...
            try {
                ...
                if (typeof callback === 'function') {
                    const result = callback(function(dep) {
                        console.warn(`[Modern Odoo17] Legacy require("${dep}") called. Consider using import statements.`);
                ...
                console.error(`[Modern Odoo17] Error in legacy module "${name}":`, error);
                return {};
            }
            ...
            load(name) {
                ...
                if (!module.instance) {
                    try {
                        ...
                        console.error(`[Modern Odoo17] Error loading module "${name}":`, error);
                        return null;
                    }
                }
    ...
    window.addEventListener('error', function(event) {
        if (event.message && event.message.includes('odoo.define is not a function')) {
            console.error('[Modern Odoo17] Legacy odoo.define() error caught:', event.message);
            ...
            if (!window.odoo.define) {
                window.odoo.define = function() {
                    console.warn('[Modern Odoo17] Emergency odoo.define() shim activated');
                    return {};
                };
            }
    ...
    window.addEventListener('unhandledrejection', function(event) {
        if (event.reason && event.reason.message && 
            ...
            console.warn('[Modern Odoo17] Module loading error handled:', event.reason.message);
            event.preventDefault();
        }
    });
...
window.getOdooService = function(serviceName) {
    if (window.odoo.serviceRegistry && window.odoo.serviceRegistry.has(serviceName)) {
