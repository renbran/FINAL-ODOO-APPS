...
(() => {
    ...
    class OSUSEnhancements {
            ...
            const scrollObserver = new IntersectionObserver((entries) => {
                entries.forEach(entry => {
                    if (entry.isIntersecting) {
                        ...
                        const items = entry.target.querySelectorAll('.list-item, .grid-item, .card');
        ...
        #createScrollProgress() {
            ...
            this.#addEventListenerTracked(window, 'scroll', scrollHandler, { passive: true });
        ...
        async #initParallaxEffects() {
            ...
            this.#addEventListenerTracked(window, 'scroll', requestParallaxUpdate, { passive: true });
        }
        ...
        async #initLuxuryHovers() {
            ...
            document.querySelectorAll('.card, .osus-dashboard-card').forEach(card => {
        ...
        #animateCardHover(card, isHover) {
            const transform = isHover ? 'translateY(-4px) scale(1.02)' : 'translateY(0) scale(1)';
        ...
        #enhanceInput(input) {
            const parent = input.closest('.form-group, .o_field_widget') || input.parentElement;
            
            input.addEventListener('focus', () => {
                parent.style.setProperty('--osus-focus-ring', '0 0 0 3px color-mix(in srgb, #4d1a1a 20%, transparent)');
            });
        ...
        async #initAutoHideNavigation() {
            ...
            this.#addEventListenerTracked(window, 'scroll', handleScroll, { passive: true });
        ...
        async #initMobileNavigation() {
            const mobileToggle = document.querySelector('.nav-mobile-toggle, .o_mobile_menu_toggle');
            ...
            const observer = new MutationObserver((mutations) => {
                mutations.forEach((mutation) => {
                    mutation.addedNodes.forEach((node) => {
                        if (node.nodeType === 1 && node.classList?.contains('o_notification')) {
        ...
        #clearFieldError(field) {
            const errorElement = field.parentElement.querySelector('.osus-field-error');
            errorElement?.remove();
        ...
        #formatDate(e) {
            ...
            if (!isNaN(date.getTime())) {
                input.setAttribute('data-formatted', 
        ...
        #updateProgress(value) {
            const progressBar = document.querySelector('.osus-loading-overlay .progress-bar');
            if (progressBar) {
        ...
        #handleResize(entries = []) {
            ...
            window.dispatchEvent(new CustomEvent('osus:resize', {
                detail: { entries }
            }));
        }
        ...
        async #initIntersectionOptimizations() {
            ...
            document.querySelectorAll('[class*="osus-"], .card, .btn').forEach(el => {
        ...
        #addEventListenerTracked(element, event, handler, options = {}) {
            ...
            this.#eventListeners.set(`${event}-${Date.now()}`, { element, event, handler });
        }
                ...
                const observer = new PerformanceObserver((list) => {
                    list.getEntries().forEach((entry) => {
                        if (entry.entryType === 'navigation' && entry.loadEventEnd > 0) {
                            console.log(`🚀 OSUS Page loaded in ${Math.round(entry.loadEventEnd)}ms`);
                        }
                    });
                });
    ...
    if (document.readyState === 'loading') {
        document.addEventListener('DOMContentLoaded', initializeOSUSEnhancements, { once: true });
    } else {
    ...
    const setupOdooIntegration = () => {
        ...
        const targetSelectors = ['.o_content', '.o_web_client', '.o_action_manager'];
    ...
    document.addEventListener('DOMContentLoaded', setupOdooIntegration, { once: true });
