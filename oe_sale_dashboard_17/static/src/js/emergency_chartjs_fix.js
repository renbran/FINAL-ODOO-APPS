...
(function() {
    ...
    function loadEmergencyChartJs() {
        return new Promise((resolve, reject) => {
            ...
            script.onload = function() {
                console.log('[CloudPepper Emergency] Local Chart.js loaded successfully');
                resolve();
            };
            script.onerror = function() {
                console.warn('[CloudPepper Emergency] Local Chart.js failed, using CDN fallback');
    ...
    function loadFromCDN() {
        return new Promise((resolve, reject) => {
            ...
            script.onerror = function() {
                console.error('[CloudPepper Emergency] All Chart.js loading methods failed');
    ...
    function createChartMock() {
        console.warn('[CloudPepper Emergency] Creating Chart.js mock to prevent crashes');
        ...
        const ChartMock = function(ctx, config) {
            ...
            if (ctx && ctx.canvas) {
                ...
                context.fillText('Chart Loading...', canvas.width/2, canvas.height/2);
            }
        };
    ...
    if (document.readyState === 'loading') {
        document.addEventListener('DOMContentLoaded', function() {
            loadEmergencyChartJs().catch(function(error) {
                console.error('[CloudPepper Emergency] Chart.js emergency loading failed:', error);
            });
        });
    } else {
        loadEmergencyChartJs().catch(function(error) {
    ...
    window.owl.Component.prototype.setup = function() {
        ...
        try {
            ...
            if (error.message && error.message.includes('chartjs')) {
                console.warn('[CloudPepper Emergency] Chart.js error in component setup, using fallback');
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    if (window.odoo && window.odoo.define) {
        window.odoo.define('web.GraphRenderer.Emergency', function(require) {
            console.log('[CloudPepper Emergency] Installing GraphRenderer emergency override');
            
            // This prevents the GraphRenderer from crashing on Chart.js load failure
            return {
                start: function() {
                    if (typeof window.Chart === 'undefined') {
                        console.warn('[CloudPepper Emergency] Chart.js not available, using placeholder');
