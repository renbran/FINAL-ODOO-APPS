...
class SaleOrderDashboardMethods(models.Model):
    """
    Extension class for sale.order to add dashboard methods
    without modifying the core sale.order model structure
    """
    _inherit = 'sale.order'

    @api.model
    def get_sales_performance_data(self, start_date, end_date):
        """Get sales performance data - delegated to dashboard model"""
        dashboard = self.env['sale.dashboard']
        return dashboard.get_sales_performance_data(start_date, end_date)

    @api.model  
    def get_monthly_fluctuation_data(self, start_date, end_date, sales_type_ids=None):
        """Get monthly fluctuation data - delegated to dashboard model"""
        dashboard = self.env['sale.dashboard']
        return dashboard.get_monthly_fluctuation_data(start_date, end_date, sales_type_ids)

    @api.model
    def get_sales_by_state_data(self, start_date, end_date):
        """Get sales by state data - delegated to dashboard model"""
        dashboard = self.env['sale.dashboard']
        return dashboard.get_sales_by_state_data(start_date, end_date)

    @api.model
    def get_top_customers_data(self, start_date, end_date, limit=10):
        """Get top customers data - delegated to dashboard model"""
        dashboard = self.env['sale.dashboard']
        return dashboard.get_top_customers_data(start_date, end_date, limit)

    @api.model
    def get_sales_team_performance(self, start_date, end_date):
        """Get sales team performance data - delegated to dashboard model"""
        dashboard = self.env['sale.dashboard']
        return dashboard.get_sales_team_performance(start_date, end_date)
