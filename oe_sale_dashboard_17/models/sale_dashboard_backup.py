...
class SaleDashboard(models.Model):
    ...
    def get_monthly_fluctuation_data(self, start_date, end_date, sales_type_ids=None):
        ...
        try:
            ...
            # Use safer date field - check if booking_date exists, otherwise use create_date
            date_field = 'booking_date' if 'booking_date' in self.env['sale.order']._fields else 'date_order'
            ...
            # Only add sales type filter if the field exists and sales_type_ids provided
            if sales_type_ids and hasattr(self, 'sale_order_type_id'):
                try:
                    base_domain.append(('sale_order_type_id', 'in', sales_type_ids))
                except:
                    _logger.warning("sale_order_type_id field not available, skipping sales type filter")
            ...
            sales_orders = self.search_read(sales_order_domain, [date_field, 'amount_total'])
            ...
            for order in sales_orders:
                if order[date_field]:
                    ...
                    if month_key in monthly_data:
                        ...
                        monthly_data[month_key]['sales_orders']['amount'] += order.get('amount_total', 0)
