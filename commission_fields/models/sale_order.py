...
class SaleOrder(models.Model):
    ...
    def _calculate_commission_amount(self, rate_field, amount_field, base_amount, type_field=None):
        """Generic method to calculate commission amount.
        
        Args:
            rate_field (str): Field name containing the rate or fixed amount
            amount_field (str): Field name where the result will be stored
            base_amount (float): Base amount for percentage calculations
            type_field (str, optional): Field name containing commission type
        ...
        commission_type = getattr(self, type_field, 'fixed') if type_field else 'fixed'
        ...
        # Store the result in the amount field
        setattr(self, amount_field, amount)
        return amount
