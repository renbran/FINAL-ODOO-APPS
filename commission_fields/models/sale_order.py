...
class SaleOrder(models.Model):
    ...
    def action_confirm_commission(self):
        """Button: Confirm commission calculation and lock values"""
        for order in self:
            if order.commission_status != 'calculated':
                raise UserError(_('Commission must be calculated before confirming.'))
    ...
    def action_reset_commission(self):
        """Button: Reset commission workflow to draft"""
        for order in self:
    ...
    def action_create_commission_purchase_order(self):
        """Button: Create a purchase order for commission payment (stub, implement as needed)"""
        # Implement logic to create a purchase order for commission if required
        # For now, just post a message
        for order in self:
            order.message_post(body=_('Commission purchase order creation triggered by %s' % self.env.user.name))
        return True

    def action_view_related_purchase_orders(self):
        """Button: View related purchase orders (stub, implement as needed)"""
        # Implement logic to return an action showing related purchase orders
        # For now, just post a message
        for order in self:
            order.message_post(body=_('View related purchase orders triggered by %s' % self.env.user.name))
        return True
