...
class SaleOrder(models.Model):
    ...
    broker_commission = fields.Monetary(string='Broker Commission', tracking=True, currency_field='currency_id', help="Commission amount for the broker")
    buyer_id = fields.Many2one('res.partner', string='Buyer', tracking=True, help="The buyer associated with this sale order", index=True)
    deal_id = fields.Char(string='Deal ID', help="Unique identifier for commission mapping.", copy=False, index=True)
    ...
    sale_value = fields.Monetary(string='Sale Value', tracking=True, currency_field='currency_id', help="The total sale value of the order", compute='_compute_sale_value', store=True, readonly=False)
    ...
    commission_payment_date = fields.Date(string='Commission Payment Date', tracking=True, copy=False)
    commission_notes = fields.Html(string='Commission Notes', tracking=True, help="Additional notes about commission calculations")
    commission_reference = fields.Char(string='Commission Reference', tracking=True, help="Reference number for commission payment")
    commission_sequence = fields.Char(string='Commission Number', copy=False, readonly=True, help="Auto-generated commission tracking number")

    # External commission fields
    external_partner_id = fields.Many2one('res.partner', string='External Partner', tracking=True, help="External partner/agent for commission")
    external_commission_type = fields.Selection(COMMISSION_TYPE_SELECTION, string='External Commission Type', default='unit_price', tracking=True, help="How to compute the external partner's commission.")
    external_percentage = fields.Float(string='External Commission %', digits=(5, 2), tracking=True, compute='_compute_commission_percentages', store=True)
    external_fixed_amount = fields.Monetary(string='External Fixed Amount', currency_field='currency_id', tracking=True)
    external_commission_amount = fields.Monetary(string='External Commission Amount', currency_field='currency_id', compute='_compute_commission_totals', store=True, tracking=True)

    broker_agency_partner_id = fields.Many2one('res.partner', string="Broker/Agency Partner", tracking=True, help="Select the broker or agency partner.")
    broker_agency_commission_type = fields.Selection(COMMISSION_TYPE_SELECTION, string="Broker/Agency Commission Type", default='unit_price', tracking=True, help="How to compute the broker/agency commission.")
    broker_agency_rate = fields.Float(string="Broker/Agency Rate", tracking=True, digits=(5, 2))
    broker_agency_total = fields.Monetary(string="Broker/Agency Total", compute='_compute_commission_totals', store=True, currency_field='currency_id', tracking=True)

    referral_partner_id = fields.Many2one('res.partner', string="Referral Partner", tracking=True, help="Select the referral partner.")
    referral_commission_type = fields.Selection(COMMISSION_TYPE_SELECTION, string="Referral Commission Type", default='unit_price', tracking=True, help="How to compute the referral commission.")
    referral_rate = fields.Float(string="Referral Rate", tracking=True, digits=(5, 2))
    referral_total = fields.Monetary(string="Referral Total", compute='_compute_commission_totals', store=True, currency_field='currency_id', tracking=True)

    cashback_partner_id = fields.Many2one('res.partner', string="Cashback Partner", tracking=True, help="Select the cashback recipient partner.")
    cashback_commission_type = fields.Selection(COMMISSION_TYPE_SELECTION, string="Cashback Commission Type", default='unit_price', tracking=True, help="How to compute the cashback commission.")
    cashback_rate = fields.Float(string="Cashback Rate", tracking=True, digits=(5, 2))
    cashback_total = fields.Monetary(string="Cashback Total", compute='_compute_commission_totals', store=True, currency_field='currency_id', tracking=True)

    other_external_partner_id = fields.Many2one('res.partner', string="Other External Partner", tracking=True, help="Select the other external commission recipient.")
    other_external_commission_type = fields.Selection(COMMISSION_TYPE_SELECTION, string="Other External Commission Type", default='unit_price', tracking=True, help="How to compute the other external commission.")
    other_external_rate = fields.Float(string="Other External Rate", tracking=True, digits=(5, 2))
    other_external_total = fields.Monetary(string="Other External Total", compute='_compute_commission_totals', store=True, currency_field='currency_id', tracking=True)

    # Internal commission fields
    internal_commission_type = fields.Selection(COMMISSION_TYPE_SELECTION, string='Internal Commission Type', default='unit_price', tracking=True, help="How to compute the internal commission for agents/managers/directors.")

    agent1_id = fields.Many2one('res.partner', string='Agent 1', tracking=True, help="Select any partner as Agent 1 for internal commission.")
    agent1_rate = fields.Float(string='Agent 1 Rate (%)', tracking=True, digits=(5, 2), compute='_compute_commission_percentages', store=True)
    agent1_fixed = fields.Monetary(string='Agent 1 Fixed Amount', tracking=True, currency_field='currency_id')
    agent1_commission = fields.Monetary(string='Agent 1 Commission', compute='_compute_internal_commissions', store=True, tracking=True, currency_field='currency_id')

    agent2_id = fields.Many2one('res.partner', string='Agent 2', tracking=True, help="Select any partner as Agent 2 for internal commission.")
    agent2_rate = fields.Float(string='Agent 2 Rate (%)', tracking=True, digits=(5, 2), compute='_compute_commission_percentages', store=True)
    agent2_fixed = fields.Monetary(string='Agent 2 Fixed Amount', tracking=True, currency_field='currency_id')
    agent2_commission = fields.Monetary(string='Agent 2 Commission', compute='_compute_internal_commissions', store=True, tracking=True, currency_field='currency_id')

    manager_id = fields.Many2one('res.partner', string='Manager', tracking=True, help="Select any partner as Manager for internal commission.")
    manager_rate = fields.Float(string='Manager Rate (%)', tracking=True, digits=(5, 2), compute='_compute_commission_percentages', store=True)
    manager_fixed = fields.Monetary(string='Manager Fixed Amount', tracking=True, currency_field='currency_id')
    manager_commission = fields.Monetary(string='Manager Commission', compute='_compute_internal_commissions', store=True, tracking=True, currency_field='currency_id')

    director_id = fields.Many2one('res.partner', string='Director', tracking=True, help="Select any partner as Director for internal commission.")
    director_rate = fields.Float(string='Director Rate (%)', tracking=True, digits=(5, 2), compute='_compute_commission_percentages', store=True)
    director_fixed = fields.Monetary(string='Director Fixed Amount', tracking=True, currency_field='currency_id')
    director_commission = fields.Monetary(string='Director Commission', compute='_compute_internal_commissions', store=True, tracking=True, currency_field='currency_id')

    # Computed summary fields
    total_internal_commission = fields.Monetary(string='Total Internal Commission', compute='_compute_total_commissions', store=True, tracking=True, currency_field='currency_id')
    total_external_commission = fields.Monetary(string='Total External Commission', compute='_compute_total_commissions', store=True, tracking=True, currency_field='currency_id')
    ...
    commission_percentage = fields.Float(string='Commission %', compute='_compute_allocation_status', store=True, digits=(5, 2), help="Commission as a percentage of the sale value")
...
class SaleOrder(models.Model):
    ...
    def _compute_commission_percentages(self):
        for order in self:
            base = order.amount_untaxed or 0.0
            # External commission percentage from fixed
            if order.external_commission_type == 'fixed' and base > 0 and order.external_fixed_amount:
                order.external_percentage = (order.external_fixed_amount / base) * 100
            # Internal commission percentages from fixed
            if order.internal_commission_type == 'fixed' and base > 0:
                if order.agent1_fixed:
                    order.agent1_rate = (order.agent1_fixed / base) * 100
                if order.agent2_fixed:
                    order.agent2_rate = (order.agent2_fixed / base) * 100
                if order.manager_fixed:
                    order.manager_rate = (order.manager_fixed / base) * 100
                if order.director_fixed:
                    order.director_rate = (order.director_fixed / base) * 100

    # Add this to the field definitions:
    external_percentage = fields.Float(
        string='External Commission %',
        digits=(5, 2),
        tracking=True,
        compute='_compute_commission_percentages',
        store=True
    )
