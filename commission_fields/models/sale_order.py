...
class SaleOrder(models.Model):
    ...
    @api.depends('external_commission_type', 'internal_commission_type')
    def _compute_show_commission_fields(self):
        for order in self:
            # External commission fields visibility
            order.show_external_percentage = order.external_commission_type in ['unit_price', 'untaxed']
            order.show_external_fixed_amount = order.external_commission_type == 'fixed'
            
            # Agent 1 fields visibility
            order.show_agent1_rate = order.internal_commission_type in ['unit_price', 'untaxed']
            order.show_agent1_fixed = order.internal_commission_type == 'fixed'
            
            # Agent 2 fields visibility
            order.show_agent2_rate = order.internal_commission_type in ['unit_price', 'untaxed']
            order.show_agent2_fixed = order.internal_commission_type == 'fixed'
            
            # Manager fields visibility
            order.show_manager_rate = order.internal_commission_type in ['unit_price', 'untaxed']
            order.show_manager_fixed = order.internal_commission_type == 'fixed'
            
            # Director fields visibility
            order.show_director_rate = order.internal_commission_type in ['unit_price', 'untaxed']
            order.show_director_fixed = order.internal_commission_type == 'fixed'
    ...
    def _compute_company_net_commission(self):
        """Compute the company net commission as the remainder after external and internal commissions."""
        for record in self:
