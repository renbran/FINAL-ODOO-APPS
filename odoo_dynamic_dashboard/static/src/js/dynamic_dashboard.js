...
export class OdooDynamicDashboard extends Component {
    ...
    async renderDashboard(){
        ...
        try {
            const themeResponse = await this.orm.call('dashboard.theme', 'get_records', [[]]);
            ...
            const dashboardResponse = await this.orm.call("dashboard.block", "get_dashboard_vals", [[], this.actionId]);
            for (let i = 0; i < dashboardResponse.length; i++) {
                ...
                if (dashboardResponse[i].type === 'tile'){
                    mount(DynamicDashboardTile, itemsContainer, { props: {
                        widget: dashboardResponse[i], doAction: this.action, dialog: this.dialog, orm: this.orm
                    }});
                }
                else{
                    mount(DynamicDashboardChart, itemsContainer, { props: {
                        widget: dashboardResponse[i], doAction: this.action, rpc: this.rpc, dialog: this.dialog, orm: this.orm
                    }});
                }
            }
        } catch (error) {
    ...
    async saveLayout(ev){
        ...
        try {
            const response = await this.orm.call('dashboard.block','get_save_layout', [[], data_list]);
    ...
    onClickAddItem(event){
        ...
        if (type == 'tile'){
            const randomColor = '#' + ('000000' + Math.floor(Math.random() * 16777216).toString(16)).slice(-6);
    ...
    async dateFilter(){
        ...
        try {
            const response = await this.orm.call("dashboard.block", "get_dashboard_vals", [[], this.actionId, start_date, end_date]);
            for (let i = 0; i < response.length; i++) {
                if (response[i].type === 'tile'){
                    mount(DynamicDashboardTile, itemsContainer, { props: {
                        widget: response[i], doAction: this.action, dialog: this.dialog, orm: this.orm
                    }});
                }
                else{
                    mount(DynamicDashboardChart, itemsContainer, { props: {
                        widget: response[i], doAction: this.action, rpc: this.rpc, dialog: this.dialog, orm: this.orm
                    }});
                }
            }
        } catch (error) {
    ...
    async clickSearch(){
        ...
        try {
            const response = await this.rpc('/custom_dashboard/search_input_chart', {'search_input': input});
