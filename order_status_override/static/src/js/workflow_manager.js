...
export class OSUSWorkflowManager extends Component {
    ...
    async loadWorkflowData() {
        ...
        try {
            const workflowData = await this.orm.call("sale.order", "get_workflow_data", [this.props.record.resId]);
            ...
            this.notification.add(_t("Failed to load workflow data: %s", error.message), { type: "danger" });
        } finally {
    ...
    async loadCommissionData() {
        try {
            this.state.commissionData = await this.orm.call("sale.order", "get_commission_data", [
    ...
    async executeWorkflowAction(action) {
        ...
        try {
            const result = await this.orm.call("sale.order", action, [this.props.record.resId]);
            ...
            this.showNotification(_t("Failed to execute action: %s", error.message), "danger");
        } finally {
            this.state.isLoading = false;
        }
    }
    ...
    async onCalculateCommission() {
        ...
        try {
            const result = await this.orm.call("sale.order", "calculate_commission_preview", [this.props.record.resId]);
            ...
            this.showNotification(_t("Failed to calculate commission: %s", error.message), "danger");
        } finally {
            this.state.isLoading = false;
        }
    }
    ...
    async onAssignUser(role, userId) {
        ...
        try {
            await this.orm.call("sale.order", "assign_user_to_role", [this.props.record.resId, role, userId]);

            this.showNotification(_t("User assigned successfully"), "success");
            await this.loadWorkflowData();
        } catch (error) {
            this.showNotification(_t("Failed to assign user: %s", error.message), "danger");
        }
    }
    ...
    get workflowProgress() {
        ...
        const currentIndex = this.state.workflowSteps.findIndex((step) => step.code === this.state.currentStatus.code);
    ...
    canPerformAction(action) {
        return this.state.nextActions.includes(action) && this.state.canTransition && !this.props.readonly;
    }
}
